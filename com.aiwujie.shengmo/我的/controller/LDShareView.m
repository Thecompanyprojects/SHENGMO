...
-(void)shareBtnAction:(UIButton *)sender{
    ...
    if ([_comeFrom isEqualToString:@"Mine"]) {
        ...
        [self createShare:sender andTitle:@"圣魔斯慕" andDescription:@"圣魔斯慕——专业亚文化交友APP诞生了！刚上线就很火爆~" andPicImage:[UIImage imageNamed:@"圣魔logo"] andWebUrl:@"www.shengmo.cn" andImageData:[NSData dataWithContentsOfURL:[NSURL URLWithString:shareUrl]] andImageUrl:[NSURL URLWithString:shareUrl]];
...
-(void)createShare:(UIButton *)button andTitle:(NSString *)title andDescription:(NSString *)description andPicImage:(UIImage *)image andWebUrl:(NSString *)webUrl andImageData:(NSData *)imageData andImageUrl:(NSURL *)imageUrl{
    ...
    if ((button.tag - 1000000) == 0) {
        ...
        _tencentOAuth = [[TencentOAuth alloc] initWithAppId:@"1105968084" andDelegate:self];
