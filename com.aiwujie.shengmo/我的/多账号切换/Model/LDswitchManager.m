...
-(void)creatTable
{
    ...
    if ([db open]) {
        //4.创表
        BOOL result=[db executeUpdate:@"CREATE TABLE IF NOT EXISTS t_discoverData1 (id INTEGER PRIMARY KEY , nickname TEXT NOT NULL, imageUrl TEXT , uid TEXT , account TEXT , password TEXT ,token TEXT ,sex TEXT , sexual ,TEXT ,way TEXT , chatstatus TEXT);"];
        if (result){
            NSLog(@"创表成功");
        }else{
            NSLog(@"创表失败");
...
-(void)insertInfowith:(LDswitchModel *)model
{
    ...
    BOOL isRollBack = NO;
    @try {
        NSString *sql = @"INSERT INTO t_discoverData1 (nickname,imageUrl,uid,account,password,token,sex,sexual,chatstatus,way) VALUES (?,?,?,?,?,?,?,?,?,?);";
...
-(BOOL)iscaninsert
{
    ...
    else
    {
        ...
        FMResultSet *resultSet = [dataBase executeQuery:@"select * from t_discoverData1"];
...
-(void)deletefromData
{
    ...
    BOOL result = [self.db executeUpdate:@"DELETE FROM t_discoverData1"];
    if (result) {
...
-(void)deletefromuid:(NSString *)uid
{
    ...
    NSString *sql = @"DELETE FROM t_discoverData1 WHERE uid = ?";
...
- (void)updateDataWithuserId:(NSString *)uid andnickname:(NSString *)nickname {
    ...
    FMDatabase *dataBase = [FMDatabase databaseWithPath:dbPath];
    if ([dataBase open]) {
        NSString *sql = @"UPDATE t_discoverData1 SET nickname = ? WHERE  uid = ?";
...
- (void)updateDataWithuserId:(NSString *)uid andimageUrl:(NSString *)imageUrl {
    ...
    FMDatabase *dataBase = [FMDatabase databaseWithPath:dbPath];
    if ([dataBase open]) {
        NSString *sql = @"UPDATE t_discoverData1 SET imageUrl = ? WHERE  uid = ?";
...
- (void)updateDataWithuserId:(NSString *)uid andpassword:(NSString *)password {
    ...
    FMDatabase *dataBase = [FMDatabase databaseWithPath:dbPath];
    if ([dataBase open]) {
        NSString *sql = @"UPDATE t_discoverData1 SET password = ? WHERE  uid = ?";
