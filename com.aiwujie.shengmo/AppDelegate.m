...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // 验证更新版本之后是否强制退出过
    // api/users/samer_logined
    // 参数uid
    // 返回：2000 code：1未登录过   2登陆过
    NSString *uid = [[NSUserDefaults standardUserDefaults] objectForKey:@"uid"];
    NSDictionary *params = @{@"uid":uid?:@""};
    NSString *url = [PICHEADURL stringByAppendingFormat:@"api/users/samer_logined"];
    [NetManager afPostRequest:url parms:params finished:^(id responseObj) {
    
        NSString *retcode = [NSString stringWithFormat:@"%@",responseObj[@"retcode"]];
        if ([retcode intValue]==2000) {
...
- (void)didReceiveMessageNotification:(NSNotification *)notification {
    ...
    if ([notification.userInfo[@"left"] intValue] == 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            //桌面角标
            [UIApplication sharedApplication].applicationIconBadgeNumber = unreadMsgCount+allnum;
        });
    }
