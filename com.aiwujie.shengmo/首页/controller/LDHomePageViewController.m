...
-(void)createData:(int)page type:(NSString *)type{
    ...
    if ([self.content intValue]==1) {
        ...
        if ([CLLocationManager authorizationStatus] ==kCLAuthorizationStatusDenied) {
            ...
            //定位不能用
            UIAlertController *control = [UIAlertController alertControllerWithTitle:@"提示" message:@"请您在手机设置-圣魔-权限中开启位置权限，以便正常使用全部功能（如您想隐藏位置，可在APP我的-设置-隐私中关闭位置）" preferredStyle:UIAlertControllerStyleAlert];
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    ...
    if ([self.content intValue] == 0 && section == 0) {
        ...
        if(_slideArray.count != 0){
            ...
            return DYANDNERHEIGHT + WARNHEIGNT + ADVERTISEMENT + OTHERHEIGHT;
        }
        
        return DYANDNERHEIGHT + WARNHEIGNT + OTHERHEIGHT;
    }
