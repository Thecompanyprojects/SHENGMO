...
-(void)createcollview:(UIView *)heiview andtophei:(CGFloat )tophei
{
    ...
    CGFloat space = 0;
    //每个小块的宽
    CGFloat itemW = (WIDTH - 2)/4;
    //每个小块的高
    CGFloat itemH = (WIDTH - 2)/4 - 20;
    //存储最后一个item的最大y值
    CGFloat lastItemY;
    
    NSArray *itemArray = @[@"gongyi1",@"gongyi2",@"gongyi3",@"gongyi7",@"gongyi5",@"gongyi6",@"bangzhu",@"gongyi8"];
    NSArray *titleArray = @[@"严格审核",@"圣魔公益",@"检测预约",@"全职招聘",@"关于我们",@"创始人",@"帮助中心",@"官方网站"];
    for (int i = 0; i < totleNum; i++) {
        ...
        UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(itemX * space + itemX * itemW, CGRectGetMaxY(heiview.frame)+tophei + bigSpace + itemY * itemH + itemY *space, itemW, itemH)];
        backView.backgroundColor = [UIColor whiteColor];
        [_scrollView addSubview:backView];
        
        //展示9宫格的图片
        //        UIImageView *itemImageView = [[UIImageView alloc] initWithFrame:CGRectMake((itemW - itemH)/2, 20, itemH, itemH)];
        ...
        [itemImageView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.mas_offset(itemH-40);
            make.height.mas_offset(itemH-40);
        }];
...
-(void)createTopicView:(CGFloat)lastItemY andBigSpace:(CGFloat)bigSpace{
    ...
    [NetManager afPostRequest:url parms:parameters finished:^(id responseObj) {
        ...
        if (integer != 2000) {
            ...
            CGFloat btnX = 5;
            UIScrollView *topicScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, lastItemY + bigSpace-6, WIDTH, 44)];
            ...
            for (int i = 0; i < [_topicArray count]; i++) {
                ...
                btn.backgroundColor = [UIColor colorWithHexString:@"0xb73acb" alpha:0.3];
...
-(void)createIntrofuceView:(CGFloat)lastItemY andBigSpace:(CGFloat)bigSpace{
    ...
    rightLab.frame = CGRectMake(15, viewSpace-6, WIDTH-40, 30);
    ...
    leftLab.frame = CGRectMake(15, viewSpace-6, 100, 30);
    ...
    rightImg.frame = CGRectMake(WIDTH-26, viewSpace+8-6, 14, 14);
    ...
    [NetManager afPostRequest:url parms:nil finished:^(id responseObj) {
        ...
        if (integer != 2000) {
            ...
            for (int i = 0; i < num1; i++) {
                ...
                UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, i * viewH + i * viewSpace+24, WIDTH, viewH)];
        ...
        if (integer != 2000) {
            ...
            for (int j = 0; j < num2; j++) {
                ...
                if (j == num2 - 1) {
                    ...
                    view2MaxY = CGRectGetMaxY(view.frame)+6;
                }
            }
...
-(void)itemButtonClick:(UIButton *)button{
    ...
    if (button.tag == 10) {
        ...
        NSString *url = [NSString stringWithFormat:@"%@%@",PICHEADURL,@"Home/info/finddetail/state/5"];
        JXBWebViewController *webVC = [[JXBWebViewController alloc] initWithURLString:url];
        webVC.title = @"圣魔公益";
        [self.navigationController pushViewController:webVC animated:YES];
    }else if (button.tag == 12){
        NSString *url = [NSString stringWithFormat:@"%@%@",PICHEADURL,@"Home/Info/news/id/29"];
        JXBWebViewController *webVC = [[JXBWebViewController alloc] initWithURLString:url];
        webVC.title = @"检测预约";
        [self.navigationController pushViewController:webVC animated:YES];
   
    }else if (button.tag == 13){
        NSString *url = [NSString stringWithFormat:@"%@%@",PICHEADURL,@"Home/info/recruit"];
        JXBWebViewController *webVC = [[JXBWebViewController alloc] initWithURLString:url];
        webVC.title = @"全职招聘";
        [self.navigationController pushViewController:webVC animated:YES];
    }else if (button.tag == 14){
        NSString *url = [NSString stringWithFormat:@"%@%@",PICHEADURL,@"Home/info/finddetail/state/6"];
        JXBWebViewController *webVC = [[JXBWebViewController alloc] initWithURLString:url];
        webVC.title = @"关于我们";
        [self.navigationController pushViewController:webVC animated:YES];
    }else if (button.tag == 15){
        NSString *url = [NSString stringWithFormat:@"%@%@",PICHEADURL,@"Home/info/finddetail/state/4"];
        JXBWebViewController *webVC = [[JXBWebViewController alloc] initWithURLString:url];
        webVC.title = @"创始人";
        [self.navigationController pushViewController:webVC animated:YES];
        ...
        NSString *url = @"http://www.shengmo.cn";
        JXBWebViewController *webVC = [[JXBWebViewController alloc] initWithURLString:url];
        webVC.title = @"官方网站";
        [self.navigationController pushViewController:webVC animated:YES];
    }
